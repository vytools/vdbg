...
export class CppVsDbgType extends vdbg_sources.LanguageDbgType {
    ...
    constructor(channel:vscode.OutputChannel, session:vscode.DebugSession, frameId:number, callback:Function) {
        ...
        try {
            vscode.workspace.findFiles('{**.cpp,**.c,**.hpp,**.h,**.cxx,**.hxx}','{.git/**}').then((files) => {
                console.log('file',files)
                this._vdbgs = vdbg_sources.search(files.map(f => f.fsPath))
